...
class TestSitemapBasic:
    ...
    def test_supabase_integration_basic(self):
        ...
        with patch(
            "src.tahecho.sitemap.supabase_integration.create_client"
        ) as mock_create:
            mock_client = MagicMock()
            mock_create.return_value = mock_client

            integration = SupabaseIntegration(
                url="https://test.supabase.co", anon_key="test_key"
            )
    ...
    def test_embedding_generator_basic(self):
        ...
        with patch("openai.AsyncOpenAI") as mock_openai:
            ...
            config = {"model": "text-embedding-3-small", "dimension": 1536}
    ...
    def test_sitemap_url_validation(self):
        ...
        with patch.dict(
            ...
            # Valid URLs
            assert (
                config_manager.validate_sitemap_url(
                    "https://docs.aleph-alpha.com/sitemap.xml"
                )
                is True
            )
            assert (
                config_manager.validate_sitemap_url("https://example.com/sitemap.xml")
                is True
            )
    ...
    def test_domain_extraction(self):
        ...
        with patch.dict(
            ...
            domain = config_manager.get_sitemap_domain(
                "https://docs.aleph-alpha.com/sitemap.xml"
            )
