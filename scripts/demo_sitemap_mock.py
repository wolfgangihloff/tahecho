...
async def demo_sitemap_functionality_mock() -> None:
    ...
    try:
        ...
        with (
            patch("tahecho.sitemap.supabase_integration.create_client") as mock_create,
            patch(
                "tahecho.sitemap.embedding_generator.openai.AsyncOpenAI"
            ) as mock_openai,
        ):
        ...
        operation_id = await scrapy_manager.scrape_sitemap(
            ...
            incremental=False,
            differential=False,
            config={"filters": ["/docs/api", "/docs/tutorial"], "max_pages": 5},
        )
        ...
        update_operation_id = await scrapy_manager.update_sitemap(
            url=sitemap_url, incremental=True, differential=False
        )
        ...
        print(
            "3. Run: python scripts/manage_sitemaps.py scrape --url https://docs.aleph-alpha.com/sitemap.xml"
        )
