...
@click.option(
    "--incremental", "-i", is_flag=True, help="Only scrape new/changed content"
)
@click.option(
    "--differential",
    "-d",
    is_flag=True,
    help="Differential update for specific sections",
)
@click.option(
    "--sections", "-s", help="Comma-separated sections for differential update"
)
...
async def scrape(ctx, url, incremental, differential, sections, max_pages):
    ...
    try:
        ...
        if differential and sections:
            sitemap_config["differential_sections"] = [
                s.strip() for s in sections.split(",")
            ]
...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Only update new/changed content",
)
@click.option(
    "--differential",
    "-d",
...
async def update(ctx, url, incremental, differential, sections):
    ...
    try:
        if url:
            ...
            if not sitemap_urls:
                click.echo(
                    "No sitemaps configured. Use --url option or configure SITEMAP_URLS."
                )
                return
            ...
            for sitemap_url in sitemap_urls:
                ...
                click.echo(
                    f"Update operation started for {sitemap_url} with ID: {operation_id}"
                )
...
@click.option(
    ...
    "-o",
    default="30d",
    help="Delete data older than (e.g., 7d, 30d, 90d)",
)
@click.option(
    "--dry-run",
    "-n",
...
def _display_operations_table(operations: List[Dict[str, Any]]):
    ...
    for op in operations:
        click.echo(
            f"{op.get('id', 'N/A'):<20} {op.get('status', 'N/A'):<15} "
