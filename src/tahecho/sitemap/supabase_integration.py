...
class SupabaseIntegration:
    ...
    def __init__(self, url: str, anon_key: str, service_key: Optional[str] = None):
        ...
        try:
            ...
            logger.info("This is normal if Supabase is not configured or there are library compatibility issues")
            self.client = None
    ...
    async def search_similar_documents(self, query_embedding: List[float], 
        ...
        try:
            ...
            if len(query_embedding) != self.embedding_dimension:
                raise ValueError(f"Query embedding dimension {len(query_embedding)} does not match expected {self.embedding_dimension}")
            ...
            query = query.rpc('match_documents', {
    ...
    async def get_document_by_url(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.client.table('documents').select('*').eq('url', url).execute()
    ...
    async def get_documents_by_source(self, source_type: str, source_id: Optional[str] = None, 
        ...
        try:
            query = self.client.table('documents').select('*').eq('source_type', source_type)
            ...
            if response.data:
                logger.info(f"Found {len(response.data)} documents for source {source_type}")
                return response.data
            else:
                return []
    ...
    async def delete_old_documents(self, source_type: str, older_than_days: int = 30) -> int:
        ...
        try:
            ...
            response = self.client.table('documents').delete().eq('source_type', source_type).lt('created_at', cutoff_date.isoformat()).execute()
            
            deleted_count = len(response.data) if response.data else 0
            logger.info(f"Deleted {deleted_count} old documents for source {source_type}")
    ...
    async def create_content_source(self, source_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = self.client.table('content_sources').insert(source_data).execute()
    ...
    async def create_sitemap_record(self, sitemap_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def get_sitemap_pages(self, sitemap_id: str, limit: int = 100) -> List[Dict[str, Any]]:
