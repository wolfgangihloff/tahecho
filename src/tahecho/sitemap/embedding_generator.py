...
class EmbeddingGenerator:
    ...
    async def generate_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            for i, embedding in enumerate(embeddings):
                if len(embedding) != self.dimension:
                    logger.warning(f"Expected embedding dimension {self.dimension}, got {len(embedding)} for text {i}")
    ...
    async def generate_content_embedding(self, content: str, max_length: int = 8000) -> List[float]:
    ...
    async def generate_combined_embedding(self, title: str, content: str, 
        ...
        for i in range(len(title_embedding)):
            combined_value = (title_weight * title_embedding[i] + 
